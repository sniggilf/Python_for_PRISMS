...
OP = os.path.join(raw_path, foldername+'_PROC\\')
if not os.path.exists(OP):
   os.makedirs(OP)

#Load external data
targ = pyvips.Image.new_from_file(foldername+'\\'+sys.argv[1], access='sequential')
...
ldist =0
tdat1,ldist, alt_t, az_t, fp_t = readheader(raw_path+'\\'+foldername+'\\'+input_name+'.dat')
...
wdist =0
wdat1,wdist,alt_wh, az_wh, fp_wh = readheader(raw_path+'\\'+foldername+'\\'+input_white+'.dat')
...
#extract image for each waveband 
#imgfin = [final_for_rgb.extract_band(i, n = 1) for i in range(len(wl_PR))]
