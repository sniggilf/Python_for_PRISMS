...
#OUTPUT_NAME.txt         - data file for Refl. spectrum
...
for i in range(1,11):
		...
		W_ll.append(Wstats(4,i)[0]-Wstats(5,i)[0])
		W_ul.append(Wstats(4,i)[0]+Wstats(5,i)[0])
...
if compare.lower() == 'y' : 
	...
	for r in refs:
		...
		yref_ll = []
		yref_ul = []
		for refi in range(len(yref)):
			yref_ll.append(yref[refi]-stdev_ref[refi])
			yref_ul.append(yref[refi]+stdev_ref[refi])
		...
		if len(xref) <=10 :
			#print(filetype)
			plt.plot(xref, yref)
			plt.fill_between(xref, yref_ll, yref_ul, alpha=0.2, label = filetype)
			#plt.errorbar(xref, yref, yerr=stdev_ref, fmt='.k', capsize=2, mfc='none', label=filetype)#r.rstrip('.'+filetype))#, color = 'blue')
		else: 
			#print(filetype)
			plt.plot(xref, yref, label=filetype)#r.rstrip('.'+filetype))
	plt.plot(wl_PR, W)
	plt.fill_between(wl_PR, W_ll, W_ul, alpha=0.2, label = output_filename)
	#plt.errorbar(wl_PR, W, yerr=Stdev, fmt='.k',  ecolor='red', capsize=2, label = output_filename)
	...
	plt.ylabel('Reflectance')
	plt.plot(wl_PR, W)
	plt.fill_between(wl_PR, W_ll, W_ul, alpha=0.2)
	#plt.errorbar(wl_PR, W, yerr=Stdev, fmt='.k',  ecolor='red', capsize=2, label = output_filename)
